...
dataset = pd.read_csv(r"F:\A水电\D454预测回归全家桶\python全家桶\python机器学习预测全家桶\电力负荷预测数据1.csv", encoding="gbk")
...
def bilstm_model():
    ...
    bilstm = Bidirectional(LSTM(128, activation='selu', return_sequences=False))(inputs)
    # 添加一个双向LSTM层。Bidirectional包装器使LSTM层可以同时处理时间序列的正向和反向。
    # 设置128个神经元，激活函数为'selu'。return_sequences=False表示该层只返回最后一个时间步的输出。

    outputs = Dense(vt_train.shape[1])(bilstm)
    # 创建一个全连接层，神经元的数量等于vt_train的列数。
...
for ii in range(n_out):
    ...
    # 设置y轴标签为"值"，字体大小为5。
    if n_out == 1:     #如果是单步预测
        plt.title(f"The prediction result of BiLSTM :\nMAPE: {mape(Ytest[:, ii], predicted_data[:, ii])} %")
    else:
        plt.title(f"{ii+1} step of BiLSTM prediction\nMAPE: {mape(Ytest[:,ii], predicted_data[:,ii])} %")
